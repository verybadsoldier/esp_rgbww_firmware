...
// the filelist is included from a file that is generated by the front-end build process
#include "./fileList.h"
...
/**
 * @brief Generates a unique identifier string.
 * 
 * this will create an id string that is compatible with the id generated by the 
 * front end application 
 * 
 * @return A `String` containing the unique identifier in the format "chipID-randomID".
 *         - `chipID`: The system's chip ID as a string.
 *         - `randomID`: An 8-digit random number as a string.
 */
String ApplicationWebserver::makeId()
{
    ...
    long min = 1;
    for (int i = 1; i < n; i++) {
        min *= 10; // Calculate 10^(n-1)
    }
    long max = min * 10 - 1; // Calculate 10^n - 1

	String chipID=String(system_get_chip_id());
	String randomID=String(random(min, max));
    return chipID + "-" + randomID;
}
